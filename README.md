...
- **Settings**: the Apple ID, an Upgrade to Pro row (see [Pro](#pro)), the reminder (a notification a day before an app stops opening if the hub could not renew it by itself; the number of days is adjustable), background renewal, a health check and the error log, and a Legal row that opens the licensing page.
...
- Settings is trimmed to what a customer needs: Account, Focusmaxxing Pro (see [Pro](#pro)), Reminder, Background refresh, Health check and Error log, Legal, and an Advanced section with Send feedback, Refresh attempts and Reset pairing file. SideStore's other rows stay in the storyboard, unseen.
...
The Upgrade to Pro row in Settings opens the plans on [focusmaxxing.net](https://focusmaxxing.net/pricing). The paid plans never sign apps for anyone: the custom apps run on your own Apple ID, the same way as the free route. More than two custom blocked apps need your own paid Apple Developer account, bought from Apple. Focusmaxxing does not provide one.
