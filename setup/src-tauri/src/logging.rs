    ...
    S: tracing::Subscriber + for<'a> LookupSpan<'a>,
{
    fn on_event(&self, event: &tracing::Event<'_>, _ctx: Context<'_, S>) {
        ...
        // the log sheet is customer-facing: the open-source projects underneath may only be named
        // on the website's licensing page, and "ipa" is not a word a customer reads. the file log
        // (lib.rs) keeps the full text for support. order matters: the longer names go first.
        let message = message
            .replace("com.SideStore.SideStore", "<hub>")
            .replace("SideStore+LiveContainer", "store app with shell")
            .replace("SideStore", "store app")
            .replace("LiveContainer", "shell app")
            .replace("AltStore", "other store app")
            .replace("StikStore", "other store app")
            .replace("focusmaxxing-hub.ipa", "the hub file")
            .replace(".ipa", " file");
