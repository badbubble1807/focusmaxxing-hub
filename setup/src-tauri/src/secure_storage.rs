// where the signing certificate and the apple sign-in state are kept between runs: the windows
// credential store when it works, a file in the app's data folder otherwise. iloader's
// secure_storage.rs with our own entry name.
...
use crate::{error::AppError, links::KEYRING_SERVICE};
