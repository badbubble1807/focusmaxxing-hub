// finding the phone over usb (through apple's usbmuxd service, which comes with itunes on windows)
// and pairing with it. iloader's device.rs; only the label the phone sees changed.
...
// the label usbmuxd files this program's connections under
const PROVIDER_LABEL: &str = "focusmaxxing-setup";
...
pub async fn list_devices() -> Result<Vec<Result<DeviceInfo, AppError>>, AppError> {
    ...
    let device_info_futures: Vec<_> = devs
        ...
        .map(|d| {
            ...
            async move {
                let provider = d.to_provider(usbmuxd_addr, PROVIDER_LABEL);
...
pub async fn get_provider_from_connection(
    ...
    let provider = device.to_provider(UsbmuxdAddr::from_env_var().unwrap(), PROVIDER_LABEL);
    Ok(provider)
}
