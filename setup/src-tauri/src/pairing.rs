// the pairing file: what lets the hub talk to the phone it is running on, through the helper.
// made here from the usb pairing (plus the newer "remote pairing" on iOS 17.4 and up, which is
// the step where the phone asks to trust the computer), kept in the credential store so plugging
// the phone in again does not ask twice, and written into the hub's Documents folder by
// place_file. iloader's pairing.rs with the manager for other apps and the export removed.
...
use crate::{
    ...
    error::AppError,
    links::{HUB_BUNDLE_ID, HUB_DISPLAY_NAME, MACHINE_NAME},
...
async fn generate_rppairing_plist(
    ...
    let bytes = generate_rppairing(provider, MACHINE_NAME).await?.to_bytes();
...
// the hub's identifier as installed on the phone (the signing step appends the team id to it), or
// None when the phone does not list it. matched by the identifier's prefix, and by the display
// name as a fallback; apps without a display name are skipped rather than failing the lookup.
pub async fn get_hub_bundle_id(device: &DeviceInfo) -> Result<Option<String>, AppError> {
    let provider = get_provider(device).await?;
    ...
    for (bundle_id, app) in installed_apps {
        if bundle_id.starts_with(HUB_BUNDLE_ID) {
            info!("found the hub on the phone as {bundle_id}");
            return Ok(Some(bundle_id));
        }
        let name = app
            .as_dictionary()
            .and_then(|x| x.get("CFBundleDisplayName").and_then(|x| x.as_string()));
        if name == Some(HUB_DISPLAY_NAME) && by_name.is_none() {
            by_name = Some(bundle_id);
        }
    }
    if let Some(bundle_id) = &by_name {
        info!("found the hub on the phone by its name as {bundle_id}");
    }
    Ok(by_name)
}
